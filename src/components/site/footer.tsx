...
export function Footer() {
  const { t } = useI18n();
  return (
    <footer className="border-t border-gold/15 bg-bg-elevated/60">
      <div className="mx-auto max-w-6xl px-4 py-14 sm:px-6">
        <div className="grid gap-10 md:grid-cols-[1.2fr_1fr_1fr]">
          <div>
            <p className="font-script text-3xl text-gold">Jelena Gutseva</p>
            <p className="mt-1 text-xs uppercase tracking-[0.28em] text-fg-muted">Mini Tattoo</p>
          ...
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-gold">{t.nav.home}</p>
          ...
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-gold">{t.footer.email}</p>
            <a
        ...
        <p className="text-center text-xs tracking-wide text-fg-subtle">{t.footer.rights}</p>
      </div>
    </footer>
  );
}
